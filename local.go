...
func (r *LocalRepository) Resolve(ref *charm.Reference) (*charm.URL, []string, error) {
	if ref.Series == "" {
	...
	if ref.Series == "bundle" {
		...
		return u.WithRevision(0), nil, nil
	}
	ch, err := r.Get(u)
	if err != nil {
		return nil, nil, err
	...
	_, err = charm.SeriesToUse(ref.Series, ch.Meta().Series)
	if err != nil {
		return nil, nil, err
	}
